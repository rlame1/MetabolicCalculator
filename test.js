            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data && data.country_code) {
                        const detected = (data.country_code === 'FI') ? 'fi' : 'en';
                        if (!localStorage.getItem('metabolic_v20_lang')) {
                ...
                // If response is not ok or country_code missing, fallback to English if not explicitly set
                if (!localStorage.getItem('metabolic_v20_lang')) {
                    document.getElementById('langSelect').value = 'en';
                    switchLanguage('en');
                }
            } catch (e) {
                // If IP check fails (timeout, adblocker, network error), default to English if not explicitly set
                if (!localStorage.getItem('metabolic_v20_lang')) {
                    document.getElementById('langSelect').value = 'en';
                    switchLanguage('en');
                }
